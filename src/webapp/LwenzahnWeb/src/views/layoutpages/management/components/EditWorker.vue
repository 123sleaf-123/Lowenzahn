...
rowData.value &&
    (
        (workerId.value = rowData.value.name),
        (workerName.value = rowData.value.userName),
        (workerPassword.value = rowData.value.password));
/**
 * @description: 获取角色列表
 * @param {*}
 * @return {*}
...
const onSubmit = () => {
    formRef.value.validate(async (valid) => {
        if (valid) {
            axios.post("http://localhost:9090/worker/updating", {
